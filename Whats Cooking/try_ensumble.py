...
testdf['ingredients_clean_string'] = [
    ' , '.join(z).strip() for z in testdf['ingredients']]
testdf['ingredients_string'] = [' '.join([WordNetLemmatizer().lemmatize(re.sub(
    '[^A-Za-z]', ' ', line)) for line in lists]).strip() for lists in testdf['ingredients']]
...
# ngram_range = ( 1 , 2 ),analyzer="word", token_pattern=r'\w+' ,
# n_features = 7000)
...
clf2 = LinearSVC(random_state=1, C=0.8, dual=False)
nb = BernoulliNB()
rfc = RandomForestClassifier(
    random_state=1, criterion='gini', n_estimators=200)
...
eclf = EnsembleClassifier(
    clfs=[clf1, clf2, nb, rfc, sgd], weights=[2, 2, 1, 2, 2])
np.random.seed(1)
for clf, label in zip([eclf],
                      ['Ensemble']):
    scores = cross_validation.cross_val_score(
        clf, predictors_tr, targets_tr, cv=2, scoring='accuracy')
    print("Accuracy: %0.4f (+/- %0.5f) [%s]" %
...
# for submit, no index
testdf[['id', 'cuisine']].to_csv("ensemble_vote1.csv", index=False)
