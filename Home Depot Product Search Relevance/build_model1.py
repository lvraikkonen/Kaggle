...
train = pd.read_csv(config.path_train).fillna('')
test = pd.read_csv(config.path_test).fillna('')
idx = test.id.values.astype(int)
y = train.relevance.values

# counts features
X1, weights, titles = (np.loadtxt(config.path_features + 'train_counts.txt'),
                       np.array(pd.read_csv(
                           config.path_features + 'weights.csv'))[:, 0],
                       np.array(pd.read_csv(config.path_features + 'titles_clean.csv'))[:, 0])
X1_test, titles_test = (np.loadtxt(config.path_features + 'test_counts.txt'),
                        np.array(pd.read_csv(config.path_features + 'titles_test_clean.csv'))[:, 0])


# similiarity features (cosine)
X4 = np.loadtxt(config.path_features + 'X_additional_train.txt')
X4_test = np.loadtxt(config.path_features + 'X_additional_test.txt')

# jaccarrd coef and dice dist features
X3 = np.loadtxt(config.path_features + 'ssfeas4train.txt')
X3_test = np.loadtxt(config.path_features + 'ssfeas4test.txt')

np.random.seed(seed=22)
p1 = build_model(titles, X1, X3, X4, titles_test, X1_test, X3_test,
                 X4_test, y, weights=weights, params=[300, 8, 0.15], top_words=10)

p2 = build_model(titles, X1, X3, X4, titles_test, X1_test, X3_test,
                 X4_test, y, weights=weights, params=[400, 4, 0.20], top_words=15)

np.savetxt(config.path_features + 'model1.txt', p1)
np.savetxt(config.path_features + 'model2.txt', p2)
